...
classdef MyZiCrossCorr < MyZiLockIn & MyDataSource & MyGuiCont
    
    properties (GetAccess = public, SetAccess = {?MyClassParser})
        n_scope = 1    % number of hardware scope
        
        n_ch1 = 1      % numbers of scope channels      
        n_ch2 = 2
        ...
        % Dead time between scope frame acquisitions. Smaller time results 
    ...
    properties (Access = private)
         ...
         PowerTr1     % Temporary variable used for averaging
         PowerTr2
         CrossCorrTr
    end
    ...
    methods (Access = public)
        ...
        function this = MyZiCrossCorr(varargin)
        ...
        function startPoll(this)
            ...
            path = sprintf('%s/channels/%i/inputselect', ...
                this.scope_path, this.n_ch1);
            ziDAQ('setInt', path, this.signal_in1-1);
            
            path = sprintf('%s/channels/%i/inputselect', ...
                this.scope_path, this.n_ch2);
            ziDAQ('setInt', path, this.signal_in2-1);
            
            % Enable data transfer from two scope channels
            ziDAQ('setInt', [this.scope_path '/channel'], 3);
            ...
            ziDAQ('set', this.scope_module, 'scopeModule/fft/power', 0)
            ziDAQ('set', this.scope_module, ...
                'scopeModule/fft/spectraldensity', 0)
